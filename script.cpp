...
void printResults(PGresult* res, int tuple, int campi) {
    std::cout << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    //QUERY 2
    {
        std::cout << "Query 2: \nElencare in ordine crescente i distretti che hanno approvato piu richieste contenenti il prodotto di nome x nel periodo che va dalla data y alla data z, dove x y e z sono parametri: " << std::endl;
    ...
    //QUERY 3
    {
        std::cout << "Query 3: \nElencare in ordine crescente le tre aziende che hanno ricevuto piu soldi da parte di WeCare e, per ognuna, il numero di preventivi vincitori che detiene:" << std::endl;

        PGresult* res = PQexec(conn, query3.c_str());
        checkResults(res, conn);
        int tuple = PQntuples(res);
        int campi = PQnfields(res);
    ...
    //QUERY 4
    {
        std::cout << "Query 4: \nElencare per ciascuna regione il tempo (in giorni) minimo, medio e massimo impiegato per portare a termine le richieste, il numero di richieste portate a termine e quelle ancora aperte: " << std::endl;

        PGresult* res = PQexec(conn, query4.c_str());
        checkResults(res, conn);
        int tuple = PQntuples(res);
        int campi = PQnfields(res);
    ...
    //QUERY 5
    {
        std::cout << "Query 5: \nPer ogni richiesta (approvata o meno) trovare il distretto che l’ha approvata e determinare durante quale riunione e con quanti voti è passata:" << std::endl;

        PGresult* res = PQexec(conn, query5.c_str());
        checkResults(res, conn);
        int tuple = PQntuples(res);
        int campi = PQnfields(res);
    ...
    //QUERY 6
    {
        std::cout << "Query 6: \nPer ciascun DISTRETTO all’interno della regione “VENETO” ricavare quante presenze ha totalizzato dall'inizio dell’anno “2020” (la somma di ogni “presenza”) e inoltre uno tra i soci meno presenti con il suo numero di presenze e lo stesso per uno tra i piú presenti:" << std::endl;

        PGresult* res = PQexec(conn, query6.c_str());
        checkResults(res, conn);
        int tuple = PQntuples(res);
        int campi = PQnfields(res);
